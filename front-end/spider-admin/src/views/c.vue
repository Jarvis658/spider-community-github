  ...
  <div>
    <div>
      <div v-if="current.avatar"  style="margin-bottom: 16px; margin-top: 10px; margin-left: 10px">
        <img :src="current.avatar" style="width: 85px; height: 85px; border-radius: 200px">
        <!-- v-if数据加载出来后再渲染 -->
...
const handleFile = function() {
    ...
    avatarForm.append("user_id",current.id);
    post("admin/info/update_avatar_c", avatarForm).then((res) => {
      alert(res.message);
      location.reload();
    });
}
