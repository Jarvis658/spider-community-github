<template>
  <div>
    <div  v-if="current.name">
      <div v-if="current.avatar" style="margin-bottom: 16px; margin-top: 10px; margin-left: 10px">
        <img  :src="current.avatar" style="
                  width: 85px;
                  height:85px;
                  border-radius: 200px;">
        <!-- v-if数据加载出来后再渲染 -->
      ...
      <div style="margin-bottom: 16px" v-if="current.name">
        <button>昵称</button>
        <input  v-model="current.name">
      </div>
      <div style="margin-bottom: 16px" v-if="current.sex">
        <button>性别</button>
        <input  v-model="current.sex">
      </div>
      <div style="margin-bottom: 16px" v-if="current.birthday2">
        <button>生日</button>
