<template>
  <div>
    ...
    <div><button @click="reject" class="btn2">reject</button></div>
     <i class="fa fa-trash-o"  @click="removePost(route.query.post_id)" style="fontSize:20px;cursor:pointer"></i>
  </div>
</template>
...
var removePost = function(postId){
    console.log(sure)
    if(sure==0)
        tip.success("确定要删帖吗？是则再点击一次");

    if (sure==1){
    post("/user/post/delete",{post_id:route.query.post_id,user_id:route.query.user_id}).then(res=>{
    tip.success("删帖成功！")
