  ...
  mounted() {
    ...
    editor.config.customUploadImg = function (resultFiles, insertImgFn) {
      ...
      var imgForm = new FormData()
        imgData.append("token",that.imgData.token)
        imgData.append("fileKey","admin/"+Math.floor(Math.random() * (999999 - 100000) + 100000) +1)
        imgData.append("image",resultFiles[0])
        post("/user/post/toQiniu", img).then((res) => {
        console.log(res);
        imgUrl = res.message;
        console.log(imgUrl)
      });
