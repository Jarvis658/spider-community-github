...
class OtherInfo extends Component {
    ...
    componentDidMount() {
        $axios.post('/user/info/view', {
            "uid": this.props.location.state.otherId,
        }).then(res => {
            console.log(res)
            this.setState({
        ...
        $axios.post('/user/interaction/followers', {
            "user_id": this.props.location.state.otherId,
    ...
    getcollect(collectgroupid) {
        ...
        Toast.show({
            ...
            afterClose: () => {
                ...
                this.props.history.push({ pathname: '/collect', state: { collectgroupid: collectgroupid } })
            },
        })
    ...
    updateinfo() {
        ...
        Toast.show({
            content: "跳转中",
            afterClose: () => {
                this.props.history.push({ pathname: '/infoupdate', state: { userId: this.props.location.state.otherId } })
            },
        })
    ...
    render() {
        return (
            <div className="P-home">
                ...
                <JumboTabs>
                    <JumboTabs.Tab title={<Space direction='vertical'><HeartOutline />关注</Space>} key='fruits' >

                        <List>
                            {this.state.fansList.map(fansList => (
                                <List.Item
                                    key={fansList.user_name}
                                    prefix={
                                        <Image
                    ...
                    <JumboTabs.Tab title={<Space direction='vertical'><SmileOutline />粉丝</Space>} key='vegetables'>
                        <List>
                        ...
                        <List>
                            {this.state.collectGroup.map(collectGroup => (
                                <List.Item
                                    ...
                                    onClick={() => this.setState({ collectgroupid: collectGroup.collect_group_id })}
                                    onClick={() => this.getcollect(collectGroup.collect_group_id)}
                    ...
                    <JumboTabs.Tab title={<Space direction='vertical'><HeartOutline />我的</Space>} key='myinfo'>
                        <List>
                            {this.state.mypost.map(mypost => (
                                ...
                                >
                                    <Space direction='vertical' onClick={()=>{this.props.history.push({pathname:"/load_post",state:{userId:this.props.location.state.userId,postId:mypost.post_id}})}}>
                                        {mypost.post_title}
                                        {/* <Space wrap>
                ...
                //     <Collapse.Panel key='2' title='关注'>
                        
                //     </Collapse.Panel>

                //     <Collapse.Panel key='3' title='收藏'>
                

                //     </Collapse.Panel>
                //     <Collapse.Panel key='4' title='历史记录'>
                //         这里是第三项的内容
                //         {this.state.collectgroupid}
                //     </Collapse.Panel>
